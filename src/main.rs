...
const HELP: &str = "eal-cli 0.1.0\n\nUsage: eal-cli <command> [options]\n\nCommands:\n  health  Check the Embedded Alerts service\n  list    List alerts\n  get     Fetch one alert; requires --id\n  watch   Stream alert events\n\nOptions:\n  -h, --help       Print this help\n  -V, --version    Print the CLI version\n\nConfiguration flags are defined in .cli-flags.toml.\n";
...
async fn main() -> anyhow::Result<()> {
    ...
    let base = env_or(&env, "EAL_BASE_URL", "http://127.0.0.1:8080");
    let timeout = env
        .get("EAL_TIMEOUT_SECONDS")
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(20);
    let output = env_or(&env, "EAL_OUTPUT", "json");
    ...
    match command.as_str() {
        ...
        "list" => {
            print_response(
                client.get(format!("{base}/v1/alerts")).send().await?,
                &output,
            )
            .await
        }
        "get" => {
            let id = env.get("EAL_ID").context("--id is required")?;
            print_response(
                client.get(format!("{base}/v1/alerts/{id}")).send().await?,
                &output,
            )
            .await
        }
...
mod tests {
    ...
    fn cli_overrides_win_without_mutating_process_environment() {
        let before = std::env::var_os("EAL_OUTPUT");
        let env = merge_env(
            EnvMap::from([("EAL_OUTPUT".into(), "text".into())]),
            [("EAL_OUTPUT".into(), "json".into())],
        );
        assert_eq!(env.get("EAL_OUTPUT").map(String::as_str), Some("json"));
        assert_eq!(env_or(&env, "EAL_OUTPUT", "text"), "json");
        assert_eq!(std::env::var_os("EAL_OUTPUT"), before);
    }

    #[test]
    fn empty_and_whitespace_env_values_are_absent() {
        for raw in ["", " ", "\t"] {
            let env = EnvMap::from([("EAL_OUTPUT".into(), raw.into())]);
            assert_eq!(env_value(&env, "EAL_OUTPUT"), None, "raw={raw:?}");
            assert_eq!(env_or(&env, "EAL_OUTPUT", "json"), "json");
        }
    }
    ...
    fn apply_flags_merges_cli_over_base_env_without_mutation() {
        let before = std::env::var_os("EAL_OUTPUT");
        let initial = EnvMap::from([("EAL_OUTPUT".into(), "text".into())]);
        let argv = vec![
            "eal-cli".into(),
            "health".into(),
            "--output".into(),
