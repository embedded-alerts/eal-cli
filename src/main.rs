...
async fn main() -> anyhow::Result<()> {
    ...
    let base = std::env::var("EAL_BASE_URL")
    ...
    match command.as_str() {
        ...
        "list" => print_response(client.get(format!("{base}/v1/alerts")).send().await?, &output).await,
        "get" => {
            let id = std::env::var("EAL_ID").context("--id is required")?;
            print_response(client.get(format!("{base}/v1/alerts/{id}")).send().await?, &output).await
        }
