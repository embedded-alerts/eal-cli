...
CliEnvValues loadFrom(String? Function(String key) lookup) {
  return CliEnvValues(
    baseUrl: _nonEmpty(lookup('EAL_BASE_URL')) ?? 'http://127.0.0.1:8080',
    output: _nonEmpty(lookup('EAL_OUTPUT')) ?? 'json',
    timeoutSeconds: _parseInt(lookup('EAL_TIMEOUT_SECONDS'), 20),
    id: _nonEmpty(lookup('EAL_ID')),
  );
}
...
Map<String, String> loadEnvMap(
  ...
  final out = <String, String>{};
  final baseUrl = pick(['EAL_BASE_URL'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'http://127.0.0.1:8080');
  if (baseUrl != null) out['EAL_BASE_URL'] = baseUrl;
  final output = pick(['EAL_OUTPUT'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'json');
  if (output != null) out['EAL_OUTPUT'] = output;
  final timeoutSeconds = pick(['EAL_TIMEOUT_SECONDS'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, '20');
  if (timeoutSeconds != null) out['EAL_TIMEOUT_SECONDS'] = timeoutSeconds;
  final id = pick(['EAL_ID'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, null);
  if (id != null) out['EAL_ID'] = id;
  return out;
}
