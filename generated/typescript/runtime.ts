...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    base_url: nonEmpty(lookup("EAL_BASE_URL")) ?? "http://127.0.0.1:8080",
    output: nonEmpty(lookup("EAL_OUTPUT")) ?? "json",
    timeout_seconds: parseIntValue(lookup("EAL_TIMEOUT_SECONDS"), 20),
    id: nonEmpty(lookup("EAL_ID")),
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const base_url = pick(["EAL_BASE_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "http://127.0.0.1:8080");
  if (base_url !== undefined) out["EAL_BASE_URL"] = base_url;
  const output = pick(["EAL_OUTPUT"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "json");
  if (output !== undefined) out["EAL_OUTPUT"] = output;
  const timeout_seconds = pick(["EAL_TIMEOUT_SECONDS"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "20");
  if (timeout_seconds !== undefined) out["EAL_TIMEOUT_SECONDS"] = timeout_seconds;
  const id = pick(["EAL_ID"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (id !== undefined) out["EAL_ID"] = id;
  return out;
}
