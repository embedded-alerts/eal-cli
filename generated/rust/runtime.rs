...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        base_url: lookup("EAL_BASE_URL").filter(|value| !value.is_empty()).unwrap_or_else(|| "http://127.0.0.1:8080".to_string()),
        output: lookup("EAL_OUTPUT").filter(|value| !value.is_empty()).unwrap_or_else(|| "json".to_string()),
        timeout_seconds: parse_int(lookup("EAL_TIMEOUT_SECONDS"), 20),
        id: lookup("EAL_ID").filter(|value| !value.is_empty()),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let base_url = pick(&["EAL_BASE_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("http://127.0.0.1:8080"));
    if let Some(value) = base_url {
        out.insert("EAL_BASE_URL".to_string(), value);
    }
    let output = pick(&["EAL_OUTPUT"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("json"));
    if let Some(value) = output {
        out.insert("EAL_OUTPUT".to_string(), value);
    }
    let timeout_seconds = pick(&["EAL_TIMEOUT_SECONDS"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("20"));
    if let Some(value) = timeout_seconds {
        out.insert("EAL_TIMEOUT_SECONDS".to_string(), value);
    }
    let id = pick(&["EAL_ID"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = id {
        out.insert("EAL_ID".to_string(), value);
    }
    Ok(out)
}
